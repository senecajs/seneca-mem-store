...
function mem_store(this: any, options: Options) {
  ...
  seneca.add(
    { role: store.name, cmd: 'count' },

    function (msg: any, reply: any) {
      if (null == msg.base) {
        return reply(new Error('Missing "base" param'))
      }

      const base: any = msg.base


      if (null == msg.name) {
        return reply(new Error('Missing "name" param'))
      }

      const name: any = msg.name


      const count = Object.keys((entmap[base] || {})[name] || {}).length
  ...
  seneca.add(
    ...
    function (msg: any, reply: any) {
      ...
      if (null == msg.q) {
        return reply(new Error('Missing "q" param'))
      }

      const q: any = msg.q


      const ent = seneca.make(base, name)
      ...
      return intern.listents(
        ...
        function (err: Error, list: any[]) {
          ...
          return reply(null, { list, count: list.length })
        }
      )
    }
  )
